...
class NaverCheckOutOrder {
  ...
  //------------------------------------------------------------------------
  function NHNPOrderSearch($data_product_id_list,$viewType = false,$action2 = '') {
    ...
    foreach($data_product_id_list as $key=>$val) {
      $request_data.="
        <mall:ProductOrderIDList>$val</mall:ProductOrderIDList>
      ";
    }
  ...
  function NHNOrder($thiskey,$action,$dday='') {
    ...
    for($i=$dday; $i>=-1; $i--) {
      ...
      $this->test = true;
      if($this->test) {
        $d_date = mktime(0,0,0,01,01,2012);
        $p_date = mktime(0,0,0,01,02,2012);
      ...
      if($data['HASMOREDATA'][0] == "true" ) {
        for($i=0; $i<12; $i++) {
          //전송데이터 셋팅
          $request_data="
          <base:InquiryTimeFrom>".$data['INQUIRYTIMEFROM']."</base:InquiryTimeFrom>
          <base:InquiryExtraData>".$data['INQUIRYEXTRADATA']."</base:InquiryExtraData>
          </mall:".$this->operation."Request>
          </soapenv:Body>
          </soapenv:Envelope>";
          $data = $this->sendNHN($request_data); // 전송
