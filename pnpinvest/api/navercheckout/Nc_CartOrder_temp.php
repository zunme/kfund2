...
//	단일배송 배송비관련 : 2011-10-27 정수진
$deliv_nobundle_cost_temp = $deliv_nobundle_cost_temp2 = $deliv_nobundle_cost_temp3 = $shippingType_temp = $shippingType_temp2 = array();
$deliv_nobundle_flag = $deliv_nobundle_cost = $nobundle_totalMoney = $deliv_nobundle_cost_ondelivery = 0;
$cldp = $deliv_nobundle_type = '';
...
for($i=0; $i<count($Cartlist); $i++) {
  ...
  if($Addition[OptCostSum] > 0 ) {
	...
	//	단일배송일 경우
	$nobundle_option = '';
	if($_SESSION[CartDelivCostType]!='2' && $Cartlist[$i][Deliv_bundle]=='1') {
		if($Cartlist[$i][Delivery_pay]=='1') { $nobundle_option = '[단일/무료]'; }
		elseif($Cartlist[$i][Delivery_pay]=='4') { $nobundle_option = '[단일/선결/'.$Cartlist[$i][Delivery_cost].']'; }
		elseif($Cartlist[$i][Delivery_pay]=='2') {
			if($Cartlist[$i][DelivTypeSelect]=='2') { $nobundle_option = '[단일/선결/'.$Cartlist[$i][Delivery_cost].']'; }
			else { $nobundle_option = '[단일/착불/'.$Cartlist[$i][Delivery_cost].']'; }
		}
		elseif($Cartlist[$i][Delivery_pay]=='3') { $nobundle_option = '[단일/착불/'.$Cartlist[$i][Delivery_text].']'; }
		else {
			echo"<script type=\"text/javascript\">
		...
		$Cartlist[$i][Goods_name] = $nobundle_option.$Cartlist[$i][Goods_name];
	}
  ...
  $item = new ItemStack($Cartlist[$i][GoodsCode], $Cartlist[$i][Goods_name], $post_tprice, $SaleCostOri[$i], $post_option, $post_count,$shopId,$CouponYn,$this->CP_CODE);
	...
	if($_SESSION[CartDelivCostType]!='2' && $Cartlist[$i][Deliv_bundle]=='1') {
		$nobundle_totalMoney += $post_tprice;
	}
  ...
  if($Cartlist[$i][PAUTOFCID] != '') {
	...
	//	일반배송
		...
		//	단일배송일 경우
		if($Cartlist[$i][Deliv_bundle]=='1') {
			$dbc = $dbc2 = 0;
			if($Cartlist[$i][Delivery_pay]=='1') { 
				$dbc = 0;
				$cldp = 1;
			}
			elseif($Cartlist[$i][Delivery_pay]=='4') { 
				$dbc = $Cartlist[$i][Delivery_cost];
				$cldp = 4;
			}
			elseif($Cartlist[$i][Delivery_pay]=='2') {
				if($Cartlist[$i][DelivTypeSelect]=='2') {
					$dbc = $Cartlist[$i][Delivery_cost];
					$cldp = 4;
				}
				else {
					$dbc2 = $Cartlist[$i][Delivery_cost];
					$cldp = 3;
				}
			}
			elseif($Cartlist[$i][Delivery_pay]=='3') {
					$dbc2 = $Cartlist[$i][Delivery_text];
					$cldp = 3;
			}
			
			$deliv_nobundle_cost_temp[] = $dbc;
			$deliv_nobundle_cost_temp3[] = $dbc2;
			$deliv_nobundle_cost_temp2[] = $cldp;
			$deliv_nobundle_flag = 1;
		}
		//	묶음배송일 경우
		else {
			//  각 상품별 배송비 처리
...
//	일반배송
	...
	*/
	...
	//	단일배송이 있을경우
	if($deliv_nobundle_flag=='1') {
		//	묶음배송 배열 재배열
		$Delivlist[goodsdelivtype] = @array_values($Delivlist[goodsdelivtype]);
		$Delivlist[goodsdelivselect] = @array_values($Delivlist[goodsdelivselect]);
		$Delivlist[goodsdelivprice] = @array_values($Delivlist[goodsdelivprice]);

		//	단일배송 배송타입 및 배송비
		$deliv_nobundle_cost = @array_sum($deliv_nobundle_cost_temp);
		$deliv_nobundle_cost_ondelivery = @array_sum($deliv_nobundle_cost_temp3);
		$deliv_nobundle_acv = @array_count_values($deliv_nobundle_cost_temp2);

		$deliv_nobundle_count = count($deliv_nobundle_cost_temp2);
		if($deliv_nobundle_count==$deliv_nobundle_acv[1]) {
			$deliv_nobundle_type = 'FREE';
		}
		elseif($deliv_nobundle_count==$deliv_nobundle_acv[3] || $deliv_nobundle_count==($deliv_nobundle_acv[3]+$deliv_nobundle_acv[1])) {
			$deliv_nobundle_type = 'ONDELIVERY';
		}
		elseif($deliv_nobundle_count==$deliv_nobundle_acv[4] || $deliv_nobundle_count==($deliv_nobundle_acv[4]+$deliv_nobundle_acv[3]+$deliv_nobundle_acv[1]) || $deliv_nobundle_count==($deliv_nobundle_acv[4]+$deliv_nobundle_acv[1]) || $deliv_nobundle_count==($deliv_nobundle_acv[4]+$deliv_nobundle_acv[3])) {
			$deliv_nobundle_type = 'PAYED';
		}
		else {
			echo"<script type=\"text/javascript\">
			  window.alert(\"배송비 처리중 오류가 발생하였습니다. 관리자에게 문의하여 주시기 바랍니다.\");
	...
	//	단일배송 상품만 있는경우
	if(count($Cartlist) == count($deliv_nobundle_cost_temp2)) {
	}
	//	단일 + 묶음 또는 묶음배송만 있는경우
	else {
		//  배송비 처리
		...
		if($acv[0] > 0) {
			...
			//	단일배송일 경우 단일배송상품 제외금액 처리 : 2011-10-27 정수진
			$all_totalMoney = 0;
			if($deliv_nobundle_flag=='1') { $all_totalMoney = $totalMoney - $nobundle_totalMoney; }
			else { $all_totalMoney = $totalMoney; }

			if($all_totalMoney >= $_POST[deliv_common][Free_line][0]) {
				$shippingPrice = 0;
				$shippingType = 'FREE';
...
if($deliv_nobundle_flag=='1') {
	...
	$shippingType_temp = array();
	if(@trim($shippingType)!='') {
		$shippingType_temp[0] = $shippingType;
	}
	$shippingType_temp[1] = $deliv_nobundle_type;
	$shippingType_temp2 = @array_count_values($shippingType_temp);

	if(count($shippingType_temp)==$shippingType_temp2[FREE]) { 
		$shippingType = 'FREE'; 
		$shippingPrice = $shippingPrice + $deliv_nobundle_cost;
	}
	elseif(count($shippingType_temp)==$shippingType_temp2[ONDELIVERY]) { 
		$shippingType = 'ONDELIVERY'; 
		$shippingPrice = $shippingPrice + $deliv_nobundle_cost_ondelivery;
	}
	else { 
		$shippingType = 'PAYED'; 
		$shippingPrice = $shippingPrice + $deliv_nobundle_cost;
