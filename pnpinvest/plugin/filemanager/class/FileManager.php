...
/* PUBLIC METHODS ****************************************************************************** */
	...
	public function initFromConfig() {
		/*사용자 호스팅설정 정보가져오기2014-12-30 임근호*/
		$sql = " select  * from  mari_mysevice_config ";
		$default = sql_fetch($sql);

		$config = parse_ini_file($this->_incPath . '/config.inc.php');
		/*디렉토리와, 기본디렉토리가 설정되어잇지 않을경우 ftp접근불가*/
		if(!$default['ftp_default_root']=="" || !$default['ftp_root']==""){
		$config[ftpHost]=$default['ftp_host'];//호스트주소
		$config[ftpUser]=$default['ftp_id'];//아이디
		$config[ftpPassword]=$default['ftp_pw'];//비번
		$config[rootDir]=$default['ftp_root']; //루트
		$config[defaultDir]=$default['ftp_default_root']; //기본루트
		$config[quota]=$default['ftp_quotat']; //FTP사용 용량제한
		}else{
		}
