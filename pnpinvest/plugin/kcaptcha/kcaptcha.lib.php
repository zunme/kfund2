...
{
    $html .= "\n".'<script>var mari_captcha_url  = "'.MARI_CAPTCHA_URL.'";</script>';
    //$html .= "\n".'<script>var mari_captcha_path = "'.MARI_CAPTCHA_PATH.'";</script>';
    $html .= "\n".'<script src="'.MARI_CAPTCHA_URL.'/kcaptcha.js"></script>';
    $html .= "\n".'<fieldset id="captcha" class="'.$class.'">';
    $html .= "\n".'<legend><label for="captcha_key">자동 가입 방지</label></legend>';
    if (MARI_S_MOBILE) $html .= '<audio src="#" id="captcha_audio" controls></audio>';
    //$html .= "\n".'<img src="#" alt="" id="captcha_img">';
    $html .= "\n".'<img src="javascript:void(0);" alt="" id="captcha_img">';
    if (!MARI_S_MOBILE) $html .= "\n".'<button type="button" id="captcha_mp3"><span></span>음성으로듣기</button>';
    $html .= "\n".'<button type="button" id="captcha_reload"><span></span>새로고침</button>';
    $html .= '<input type="text" name="captcha_key" id="captcha_key" required class="captcha_box required" size="6" maxlength="6">';
    $html .= "\n".'<span id="captcha_info">숫자를 입력하여 주십시오.</span>';
    $html .= "\n".'</fieldset>';
    return $html;
}
