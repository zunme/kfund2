	...
	<div id="container">
		...
		<ul class="tab_btn1 pl20">
		 ...
		 <div class="local_ov01 local_ov">
			<a href="{MARI_HOME_URL}/?cms=loans" class="ov_listall">전체목록</a>  입금 : <?php echo $total_count;?>명
		</div>

		<form  class="local_sch01 local_sch"  id="fsearch" name="fsearch"  method="get">
		<input type="hidden" name="cms" value="loans">
		...
		<div class="tbl_head01 tbl_wrap mt20">
			<table class="txt_c">
				...
				if($orders_list['o_status']=="입금완료"){
					$정상자산합계 += $orders_list['o_ln_money'];
				}
				$연체자산합계 += $orders_list['o_odinterest'];
				$대출잔고 += $orders_list['o_mh_money'];
				   }
				   if ($i == 0)
				      echo "<tr><td colspan=\"7\">대출자산내역이 없습니다.</td></tr>";
				?>
				</tbody>
			</table>
