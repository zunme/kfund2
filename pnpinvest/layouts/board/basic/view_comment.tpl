...
-->
										...
										<?php }else{?>
											...
											<a href="{MARI_HOME_URL}/?mode=bbs_view&table=qna&subject=질문과답변&id=<?php echo $row['w_id']?>&type=view&reple=modi&co_id=<?php echo $row[co_id];?>">수정</a>
											<a href="{MARI_HOME_URL}/?up=bbs_comment&w_id=<?php echo $row[w_id];?>&co_id=<?php echo $row[co_id];?>&type=d" title="delete">삭제</a>
											<?php }else{?>
											<?php }?>
										<?php }?>
							...
							<div class="comment_wrap">  
								...
								<input type="hidden" name="co_id" value="<?php echo $co_id;?>">
									...
									<div class="notice_comment2_cont">
										...
										<?php if($reple=="modi"){?>
											<textarea name="co_content" id="co_content"   onkeyup="updateChar_co(<?php if(!$user[m_id]){?>1<?php }else{?>300<?php }?>, 'co_content', 'chkBite_comment');"><?=$ment_modi['co_content']?></textarea>																				
											<a href="javascript:void(0);" onclick="reple_modi()"><img src="{MARI_HOMESKIN_URL}/img/btn_register1.png" style="cursor:pointer;" alt="등록"/></a>
										<?php }else{?>
											<textarea name="co_content" id="co_content"   onkeyup="updateChar_co(<?php if(!$user[m_id]){?>1<?php }else{?>300<?php }?>, 'co_content', 'chkBite_comment');"><?=$ser['co_content']?></textarea>						
