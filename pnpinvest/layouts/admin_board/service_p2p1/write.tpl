	...
	<div id="client_container">
		<div class="client_containerinner">
			<h3 class="inquiry">
				문의하기
				<ul>
					<li><?php echo $subject?> > <?php echo $ptype;?></li>
				</ul>
			</h3>
				<div class="receipt_write">
					...
					<input type="hidden" name="skin" value="<?php echo $skin?>"><!--스킨설정-->
					<input type="hidden" name="type" value="default">
					<input type="hidden" name="w_catecode" value="<?php echo $ptype;?>">
					...
					<col width="*">
						...
						<tr>
							<th>회사명</th>
							<td><input type="text" id="w_1" name="w_company_name" value="<?php echo $master['service_company_name'];?>" readonly><!--(주)인투윈 소프트--></td>
						...
						<tr>
							...
							<td ><input type="text" id="w_2" name="w_hp" value="<?php echo $w['w_hp'];?>"></td>
						...
						<tr class="urgency_request">
							<th>긴급요청</th>
							<td colspan="3">
								<p>
									<input type="checkbox"   name="urgency" value="Y"><label for="c_4"><span></span>긴급처리 요청</label>
								</p>
								<p>
									<img class="urgency" src="{MARI_ADMINSKIN_URL}/img/ico_key.png" alt="">
									<span class="c_4_01">불필요하게 긴급 요청을 많이 하시는 경우, 업무 우선 순위에서 불이익을 받을 수 있습니다.<br/>꼭 필요한 경우에만 긴급요청을 하여 주시기 바랍니다.</span>
									<span class="request">
										<b>긴급처리 요청사항은 웹사이트 및 시스템에 치명적인 오류로 인하여
												수정이 안될 경우 운영에 큰 지장이 있을 경우에만 체크하시기 바랍니다.</b>
										<b class="close"><img src="{MARI_ADMINSKIN_URL}/img/img_close.png" alt=""></b>
									</span>
								</p>
							</td>
						</tr>
...
<?php if(!$member_ck){?>
	...
	if(!f.w_hp.value){alert('\n휴대폰번호를 입력하여 주십시오.');f.w_hp.focus();return false;}
...
<?php } ?>
	...
	if(inter_pattern.test(f.w_hp.value)){alert('\n휴대폰번호는 숫자만 입력하실수 있습니다');f.w_hp.value='';f.w_hp.focus();return false;}
	...
	f.method= 'post';
	f.action='{MARI_HOME_URL}/sql/imwork.php?update=cs_bbs_write&type=default&member_ck=<?php echo $user[m_id]?>';
	f.submit();
}
</script>
