...
<?php if($mode=="main"){?>
<?php }else{?>
  <script type="text/javascript" src="{MARI_MOBILESKIN_URL}/js/jquery.vticker-min.js"></script>
  <script>
			$(function(){  
				$('.dv_rolling').vTicker({
					// 스크롤 속도(default: 700)  
...
<!--리얼타임 데이타-->
	<div class="realtime_wrap">
		<div class="realtime1" id="realtime1"  style="display:none;">
			<h4 class="realtime_title1">REAL TIME <span>MONEY PLUS</span></h4>
			<div class="realtime1_inner">
				<div class="dv_rolling">
					...
					<?php if($realtimeitem_display_01=="Y"){?>
						...
						?>
								<li>
									<img src="{MARI_MOBILESKIN_URL}/img/real_sort3.png" alt="투자" />
									<a href="{MARI_HOME_URL}/?mode=invest_view&loan_id=<?php echo $row['loan_id'];?>"><?php if($displayprofile_use_02=="Y"){?>[<?php echo $row['i_goods'];?>]<?php }?> ‘<?=cut_str(strip_tags($row['i_subject']),14,"…")?>’<?php if($displayprofile_use_01=="Y"){?><?php echo Name_conf($row['m_name']);?>님<?php }?> 입찰 완료</a>
					...
					<?php if($realtimeitem_display_02=="Y"){?>
						...
						?>
								<li>
									<img src="{MARI_MOBILESKIN_URL}/img/real_sort2.png" alt="대출" />
									<a href="{MARI_HOME_URL}/?mode=invest_view&loan_id=<?php echo $row['i_id'];?>"><?php if($displayprofile_use_02=="Y"){?>[<?php echo $row['i_payment'];?>]<?php }?> ‘<?=cut_str(strip_tags($row['i_subject']),14,"…")?>’<?php if($displayprofile_use_01=="Y"){?><?php echo Name_conf($row['m_name']);?>님<?php }?> 대출</a>
									<?php if($displayprofile_use_03=="Y"){?><span><?php echo substr($row['i_regdatetime'],0,10); ?></span><?php }?>
								</li>
						<?php }?>
					<?php }?>
					...
					?>
						<?php if($row['o_status']=="입금완료" && $realtimeitem_display_03=="Y"){?>
							<li>
								<img src="{MARI_MOBILESKIN_URL}/img/real_sort1.png" alt="입금완료" />
						...
						<?php }else if($row['o_salestatus']=="정산완료" && $realtimeitem_display_04=="Y"){?>
							<li>
								<img src="{MARI_MOBILESKIN_URL}/img/real_sort4.png" alt="정산완료" />
			...
			<div class="close_btn2"><a href="javascript:click_btns('realtime2')"><img src="{MARI_MOBILESKIN_URL}/img/btn_close2.png" alt="닫기" /></a></div>
		</div><!-- /realtime1 -->
		<div class="realtime2" id="realtime2">
			<div class="open_btn2"><a href="javascript:click_btns('realtime1')"><img src="{MARI_MOBILESKIN_URL}/img/btn_open2.png" alt="REAL TIME MONEY PLUS 열기" /></a></div>
		</div>
	</div><!-- /realtime_wrap -->
<?php }?>