...
// 게시글리스트 썸네일 생성
function get_list_thumbnail($w_table, $w_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=true, $crop_mode='center', $is_sharpen=true, $um_value='80/0.5/3')
{
    global $mari, $config;
    $filename = $alt = "";
    $edt = false;

    $sql = " select bf_file, bf_content from {$mari['board_file_table']}
                where w_table = '$w_table' and w_id = '$w_id' and bf_type between '1' and '3' order by bf_no limit 0, 1 ";
    $row = sql_fetch($sql);

    if($row['bf_file']) {
        $filename = $row['bf_file'];
        $filepath = MARI_DATA_PATH.'/file/'.$w_table;
        $alt = get_text($row['bf_content']);
    } else {
        $write_table = $mari['write_prefix'].$w_table;
        $sql = " select wr_content from $write_table where w_id = '$w_id' ";
        $write = sql_fetch($sql);
        ...
        for($i=0; $i<count($matches[1]); $i++)
        {
            ...
            if(strpos($p['path'], '/'.MARI_DATA_DIR.'/') != 0)
                $data_path = preg_replace('/^\/.*\/'.MARI_DATA_DIR.'/', '/'.MARI_DATA_DIR, $p['path']);
            else
                $data_path = $p['path'];

            $srcfile = MARI_PATH.$data_path;
    ...
    if($tname) {
        if($edt) {
            $src = MARI_HOME_URL.str_replace($filename, $tname, $data_path);
        } else {
            $src = MARI_DATA_URL.'/file/'.$w_table.'/'.$tname;
        }
    } else {
        return false;
    }
...
function get_view_thumbnail($contents, $thumb_width=0)
{
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
            ...
            if ($width) {
                $thumb_tag = '<img src="'.MARI_HOME_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'" width="'.$width.'" height="'.$height.'"/>';
            } else {
                $thumb_tag = '<img src="'.MARI_HOME_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'"/>';
            }

            // $img_tag에 editor 경로가 있으면 원본보기 링크 추가
            $img_tag = $matches[0][$i];
            if(strpos($img_tag, 'data/editor') && preg_match("/\.({$config['cf_image_extension']})$/i", $filename)) {
                $imgurl = str_replace(MARI_HOME_URL, "", $src);
                $thumb_tag = '<a href="'.MARI_B_URL.'/view_image.php?fn='.urlencode($imgurl).'" target="_blank" class="view_image">'.$thumb_tag.'</a>';
            }
...
function thumbnail($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=true, $um_value='80/0.5/3')
{
    ...
    if (!is_dir($target_path)) {
        @mkdir($target_path, MARI_DIR_PERMISSION);
        @chmod($target_path, MARI_DIR_PERMISSION);
    }
    ...
    if(!defined('MARI_THUMB_JPG_QUALITY'))
        $jpg_quality = 90;
    else
        $jpg_quality = MARI_THUMB_JPG_QUALITY;

    imagejpeg($dst, $thumb_file, $jpg_quality);
    chmod($thumb_file, MARI_FILE_PERMISSION); // 추후 삭제를 위하여 파일모드 변경
